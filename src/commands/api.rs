...
pub async fn update_role(info: RoleParams) -> Result<()> {
    let resp = CLIENT
        .patch(
            env::var("API_URL").expect("API_URL environment var has not been set.")
                + "/api/v1/guild/register",
    ...
    match resp.status().into() {
        200 => {
            let resp = resp.json::<RoleResult>().await?.success;
            ensure!(
                resp,
                "Could not update the role of guild with id of {}",
                info.id
            );
            Ok(())
        }
        404 => Err(anyhow!("Guild with id of {} could not be found.", info.id)),
        401 => Err(AuthError::Incorrect.into()),
        400 => Err(AuthError::None.into()),
        _ => Err(anyhow!("Unknown error: {resp:?}")),
    }
